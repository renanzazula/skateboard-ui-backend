...
`scripts/check-vendored-specs.sh` is the drift alarm: with the sibling repos checked out under the same parent, it diffs each vendored spec against the upstream's `api/openapi.yaml` (ignoring the vendor-note header and `info.title`). The `app-config-openapi.yaml` copy is clean; the podcast/user/notification copies carry known hand edits that predate the script, so run it `--warn-only` in CI until those are reconciled.
...
- `HomeFeaturedPlayerService.getFeaturedPlayer()` (`GET /api/home/featured-player`) reads the Featured Player config from app-config-be, then resolves it via a `FeaturedContentResolver` (one implementation per `FeaturedContentSource` — today just `PodcastFeaturedContentResolver`, registered as a `@Component` and injected as `List<FeaturedContentResolver>`). Add a new source by adding a new resolver, not by branching inside the service. Any failure to resolve (disabled, unconfigured, downstream unavailable, referenced content deleted/unpublished) returns `null` → controller relays `204`, never an error — the rest of Home must stay usable regardless of this feature's state.
  - **MANUAL vs AUTO selection** (config field `selectionMode`, owned by app-config-be): MANUAL calls `resolver.resolve(contentId, preferredPlatform)` exactly as before — the admin-picked post. AUTO calls `resolver.resolveAuto(preferredPlatform)` instead and ignores `contentId` entirely — app-config-be never persists one for AUTO, since the "current" episode is decided live, on every read, not stored. `PodcastFeaturedContentResolver.resolveAuto` gets its post from `PodcastClient.getFeaturedEpisode()` (podcast-be's `GET /api/podcast/featured-episode`: the latest published, YouTube-sourced post matching the official "Skateboard Podcast #<n>" title pattern), then resolves playback the same way `resolve()` does for a manually-picked post. A missing/`null` `selectionMode` (a config response from before this field existed) is treated as MANUAL — required so an existing manual configuration keeps working unchanged after this deploy. `resolveAuto` returning `null` (nothing currently qualifies) is routine and just omits the player, same as any other "can't resolve" case; a genuine downstream outage from `getFeaturedEpisode()` is NOT swallowed, mirroring how a non-404 `DownstreamServiceException` from `getById` already propagates out of `resolve()`.
